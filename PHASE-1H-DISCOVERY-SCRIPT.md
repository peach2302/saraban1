# Phase 1H — Discovery Script (Complete Source Code)
...
function validateUsers(sheet: SheetData): {  SheetData; validation: UsersValidation } {
...
function validatePositions(sheet: SheetData): {  SheetData; validation: PositionsValidation } {
  const positionIds = sheet.rows.map(r => r.Position_ID).filter(Boolean);
  const departmentIds = sheet.rows.map(r => r.Department_ID).filter(Boolean);
...
function validateDepartments(sheet: SheetData): {  SheetData; validation: DepartmentsValidation } {
  const departmentIds = sheet.rows.map(r => r.Department_ID).filter(Boolean);
...
function validateUnits(sheet: SheetData): {  SheetData; validation: UnitsValidation } {
  const unitIds = sheet.rows.map(r => r.Unit_ID).filter(Boolean);
  const departmentIds = sheet.rows.map(r => r.Department_ID).filter(Boolean);
